(function(){
  ...
  const CONFIG = {
    ...
    detailsUrl: "http://ocasamento.website/pages/casamento",
    assets: {
  ...
  function initDaisies(){
    ...
    const sources = (CONFIG.assets.daisies || []).map(absUrl).filter(Boolean);
    const fallback = absUrl("../imagens/daisy.png");

    const getSrc = () => (sources.length ? pick(sources) : fallback) || fallback;
    ...
    function farEnough(x, y, dist){
      for(let i = 0; i < positions.length; i++){
        const p = positions[i];
        if(Math.hypot(p.x - x, p.y - y) < dist) return false;
      }
      return true;
    }
  ...
  function initInvite(){
    ...
    if(!root.getAttribute("data-details-url")){
      root.setAttribute("data-details-url", CONFIG.detailsUrl);
    }

    const detailsUrl = root.getAttribute("data-details-url") || CONFIG.detailsUrl;
    ...
    const mainLabel = mainBtn ? mainBtn.querySelector(".ii__btn-label") : null;
    ...
    function clearOpenTimers(){
      while(openTimers.length) clearTimeout(openTimers.pop());
    }
    ...
    function startCountdown(seconds){
      ...
      setButton(`${CONFIG.labels.details} (${countdownLeft})`, true);

      countdownTimer = setInterval(() => {
        countdownLeft -= 1;
        if(countdownLeft <= 0){
          clearCountdown();
          setButton(CONFIG.labels.details, false);
          return;
        }
        setButton(`${CONFIG.labels.details} (${countdownLeft})`, true);
      }, 1000);
    ...
    function ensureEnvelopePiece(){
      ...
      const src = envelopePiece.getAttribute("src") || "";
      if(!src){
    ...
    function goFront(){
      ...
      showButton(false);
      setButton(CONFIG.labels.open, false);
      showRestart(false);
